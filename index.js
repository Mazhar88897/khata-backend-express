...
app.post("/api/profiles", async (req, res) => {
  try {
    ...
    if (totalProfiles >= 8) {
      return res
        .status(403)
        .json({ message: "Maximum allowed profiles reached" });
    }
...
app.put(
  "/api/customers/:customerId/edit-amount/:amountId",
  async (req, res) => {
    try {
...
app.delete(
  "/api/customers/:customerId/delete-amount/:amountId",
  async (req, res) => {
    try {
      const { customerId, amountId } = req.params;
...
// Edit Amount for a Customer
app.put(
  "/api/suppliers/:supplierId/edit-amount/:amountId",
  async (req, res) => {
    try {
...
app.delete(
  "/api/suppliers/:supplierId/delete-amount/:amountId",
  async (req, res) => {
    try {
      const { supplierId, amountId } = req.params;
...
app.get("/api/accounts/by-profile/:profileId", async (req, res) => {
  try {
    const { profileId } = req.params;
    const accounts = await Account.find({ profileId }).populate(
      "customers suppliers"
    );
    res.status(200).json(accounts);
  } catch (error) {
